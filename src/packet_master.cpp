...
DeserializerFreeBits* deserializer_get_free_bits(Deserializer* deserializer, Result* result) {
   ...
   if (free_bits == NULL) {
        uint8_t* byte = deserializer->reader->read(1);
        if (byte == NULL) {
...
uint16_t deserialize_uint16_max(Deserializer* deserializer, uint8_t max_bits, Result* result) {
    ...
    if (max_bits <= BYTE_SIZE) {
        ...
        uint8_t* bytes = deserializer->reader->read(byte_count);
        if (bytes == NULL) {
            result->status = ResultStatus::ReadFailed;
            return 0;
        }
...
void flush_buffer(Serializer* serializer, Result* result) {
    ...
    if (free_bits == NULL) {
        ...
        int write_result = serializer->writer->write(serializer->buffer.data, serializer->buffer.size);
        if (write_result != 0) {
        ...
        if (count > 0) {
            int write_result = serializer->writer->write(serializer->buffer.data, count);
            if (write_result != 0) {
                result->status = ResultStatus::WriteFailed;
                result->error_info.write_error = write_result;
            }
